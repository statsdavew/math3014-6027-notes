...
```{r frf2-block-1}
block1 <- FrF2::FrF2(nruns = 8, nfactors = 3, blocks = 2, 
                     alias.info = 3, randomize = F)
block1 <- data.frame(model.matrix(~ Blocks + (A + B + C)^3, block1))
...
```
```{r frf2-block-2}
block2 <- FrF2::FrF2(nruns = 8, nfactors = 3, blocks = 4, 
                     alias.info = 3, randomize = F, alias.block.2fis = T)
block2 <- data.frame(model.matrix(~ Blocks + (A + B + C)^3, block2))
