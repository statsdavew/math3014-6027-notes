...
\right.
\end{equation}
for $j=1,\dots,n$.^[We will discuss the choice of *coding* -1, +1 later.]
...
### Analysis of Variance and R$^{2}$ as Model Comparison
...
1. [Adapted from @Morris2011] A classic and famous example of a simple hypothetical experiment was described by @Fisher1935:
    ...
    d. Suppose eight tea cups are available for this experiment but they are not identical. Instead they come from two sets. Foru are made from heavy, thick porcelain; four from much lighter china. If each cup can only be used once, how might this fact be incorporated into the design of the experiment?
...
2. Consider the linear model
    ...
    $$\mbox{Var}(\hat{\bbeta}) = \left(X^{\rm{T}}X\right)^{-1}\sigma^2\,.$$
...
a. The method of least squares minimises the sum of squared differences between the responses and the expected values, that is, minimises the expression
    ...
    $$
    (\bY-X\bbeta)^{\rm{T}}(\bY-X\bbeta) = \bY^{\rm{T}}\bY - 2\bbeta^{\rm{T}}X^{\rm{T}}\bY + \bbeta^{\rm{T}}X^{\rm{T}}X\bbeta\,.
    $$
    Differentiating with respect to the vector $\bbeta$, we obtain

    $$
    \frac{\partial}{\partial\bbeta} = -2X^{\rm{T}}\bY + 2X^{\rm{T}}X\bbeta\,.
    $$

    Set equal to $\boldsymbol{0}$ and solve:

    $$
    X^{\rm{T}}X\hat{\bbeta} = X^{\rm{T}}\bY \Rightarrow \hat{\bbeta} = \left(X^{\rm{T}}X\right)^{-1}X^{\rm{T}}\bY\,.
    $$

    The estimator $\hat{\bbeta}$ is unbiased:

    $$
    E(\hat{\bbeta}) = \left(X^{\rm{T}}X\right)^{-1}X^{\rm{T}}E(\bY) = \left(X^{\rm{T}}X\right)^{-1}X^{\rm{T}}X\bbeta = \bbeta\,,
    $$
    
    and has variance:

    \begin{align*}
\mbox{Var}(\hat{\bbeta}) & =E\left\{ \left[\hat{\bbeta} - E(\hat{\bbeta})\right] \left[\hat{\bbeta} - E(\hat{\bbeta})\right]^{\rm{T}} \right\}\\
 & =  E\left\{ \left[\hat{\bbeta} - \bbeta\right] \left[\hat{\bbeta} - \bbeta\right]^{\rm{T}} \right\}\\
 & = E\left\{ \hat{\bbeta}\hat{\bbeta}^{\rm{T}} - 2\bbeta\hat{\bbeta}^{\rm{T}} + \bbeta\bbeta^{\rm{T}} \right\}\\
 & = E\left\{ \left(X^{\rm{T}}X\right)^{-1}X^{\rm{T}}\bY\bY^{\rm{T}}X\left(X^{\rm{T}}X\right)^{-1}  - 2\bbeta \bY^{\rm{T}}X\left(X^{\rm{T}}X\right)^{-1} + \bbeta\bbeta^{\rm{T}}\right\}\\
 & = \left(X^{\rm{T}}X\right)^{-1}X^{\rm{T}}E(\bY\bY^{\rm{T}})X\left(X^{\rm{T}}X\right)^{-1} - 2\bbeta E(\bY^{\rm{T}})X\left(X^{\rm{T}}X\right)^{-1} + \bbeta\bbeta^{\rm{T}}\\
 & = \left(X^{\rm{T}}X\right)^{-1}X^{\rm{T}}\left[\mbox{Var}(\bY) + E(\bY)E(\bY^{\rm{T}})\right]X\left(X^{\rm{T}}X\right)^{-1} - 2\bbeta\bbeta^{\rm{T}}X^{\rm{T}}X\left(X^{\rm{T}}X\right)^{-1} + \bbeta\bbeta^{\rm{T}}\\
 & = \left(X^{\rm{T}}X\right)^{-1}X^{\rm{T}}\left[I_N\sigma^2 + X\bbeta\bbeta^{\rm{T}}X^{\rm{T}}\right]X\left(X^{\rm{T}}X\right)^{-1} - \bbeta\bbeta^{\rm{T}}\\
...
a. As $\bY\sim N\left(X\bbeta, I_N\sigma^2\right)$, the likelihood is given by
    ...
    $$
    L(\bbeta\,; \bY) = \left(2\pi\sigma^2\right)^{-N/2}\exp\left\{-\frac{1}{2\sigma^2}(\bY - X\bbeta)^{\rm{T}}(\bY - X\bbeta)\right\}\,.
    $$

    The log-likelihood is given by

    $$
    l(\bbeta\,;\bY) = -\frac{1}{2\sigma^2}(\bY - X\bbeta)^{\rm{T}}(\bY - X\bbeta) + \mbox{constant}\,.
    $$
...
3. Consider the two nested linear models
    ...
    **Hint:** which residual sum of squares are you interested in to compare models (i) and (ii)?
    ...
    | Total | $n-1$ | $\bY^{\rm{T}}\bY - n\bar{Y}^2$ | | 
...
a. From lectures
    ...
    | Source | Degrees of freedom | Sums of squares | Mean square |
    | :----- | :----------------: | :-------------: | :---------: |
    | Regression | $p_1+p_2$ | $\hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - n\bar{Y}^2$ | $\left(\hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - n\bar{Y}^2\right)/(p_1+p_2)$ |
    | Residual | $n-p_1-p_2-1$ | $(\bY - X\hat{\bbeta})^{\rm{T}}(\bY - X\hat{\bbeta})$ | RSS$/(n-p_1-p_2-1)$ |
    | Total | $n-1$ | $\bY^{\rm{T}}\bY - n\bar{Y}^2$ | | 

    where 

    \begin{align*}
\mbox{RSS(null) - RSS(ii)} & = \bY^{\rm{T}}\bY - n\bar{Y}^2 - (\bY - X\hat{\bbeta})^{\rm{T}}(\bY - X\hat{\bbeta})\\
& = \bY^{\rm{T}}\bY - n\bar{Y}^2 - \bY^{\rm{T}}\bY + 2\bY^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta}\\
& = 2\hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - n\bar{Y}^2\\
& = \hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - n\bar{Y}^2\,.
    \end{align*}

a. To compare model (i) with the null model, we compute

    \begin{align*}
    \mbox{RSS(null) - RSS(i)} & = \bY^{\rm{T}}\bY - N\bar{Y}^2 - (\bY - X_1\hat{\bbeta}_1)^{\rm{T}}(\bY - X_1\hat{\bbeta}_1)\\
    & = \hat{\bbeta}_1^{\rm{T}}X_1^{\rm{T}}X_1\hat{\bbeta}_1 - n\bar{Y}^2\,.
    \end{align*}
    ...
    & = (\bY - X_1\hat{\bbeta}_1)^{\rm{T}}(\bY - X_1\hat{\bbeta}_1) - (\bY - X\hat{\bbeta})^{\rm{T}}(\bY - X\hat{\bbeta})\\
    & = \hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}_1^{\rm{T}}X_1^{\rm{T}}X_1\hat{\bbeta}_1\,.
    \end{align*}

    | Source | Degrees of freedom | Sums of squares | Mean square |
    | :----- | :----------------: | :-------------: | :---------: |
    | Regression | $p_1+p_2$ | $\hat{\bbeta}X^{\rm{T}}X\hat{\bbeta} - n\bar{Y}^2$ | $\left(\hat{\bbeta}X^{\rm{T}}X\hat{\bbeta} - n\bar{Y}^2\right)/(p_1+p_2)$ |
    | Model (i) | $p_1$ | $\hat{\bbeta}_1^{\rm{T}}X_1^{\rm{T}}X_1\hat{\bbeta}_1 - n\bar{Y}^2$ | $\left(\hat{\bbeta}_1^{\rm{T}}X_1^{\rm{T}}X_1\hat{\bbeta}_1 - n\bar{Y}^2\right)/p_1$ |
    | Extra due to Model (ii) | $p_2$ | $\hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}_1^{\rm{T}}X_1^{\rm{T}}X_1\hat{\bbeta}_1$ | $\left(\hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}_1^{\rm{T}}X_1^{\rm{T}}X_1\hat{\bbeta}_1\right)/p_2$ |
    | Residual | $n-p_1-p_2-1$ | $(\bY - X\hat{\bbeta})^{\rm{T}}(\bY - X\hat{\bbeta})$ | RSS$/(n-p_1-p_2-1)$ |
    | Total | $n-1$ | $\bY^{\rm{T}}\bY - n\bar{Y}^2$ | | 
...
& = (\bY - X_2\hat{\bbeta}_2)^{\rm{T}}(\bY - X_2\hat{\bbeta}_2) - (\bY - X\hat{\bbeta})^{\rm{T}}(\bY - X\hat{\bbeta})\\
& = \hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}_2^{\rm{T}}X_2^{\rm{T}}X_2\hat{\bbeta}_2\,.
    \end{align*}
    ...
    | :----: | :----------------: | :-------------: | :---------: |
    | Regression | $p_1+p_2$ | $\hat{\bbeta}X^{\rm{T}}X\hat{\bbeta} - n\bar{Y}^2$ | $\left(\hat{\bbeta}X^{\rm{T}}X\hat{\bbeta} - n\bar{Y}^2\right)/(p_1+p_2)$ |
    | Extra Model (i) | $p_1$ | $\hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}_2^{\rm{T}}X_2^{\rm{T}}X_2\hat{\bbeta}_2$ | $\left(\hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}_2^{\rm{T}}X_2^{\rm{T}}X_2\hat{\bbeta}_2\right)/p_1$ |
    | Extra Model (ii) | $p_2$ | $\hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}_1^{\rm{T}}X_1^{\rm{T}}X_1\hat{\bbeta}_1$ | $\left(\hat{\bbeta}^{\rm{T}}X^{\rm{T}}X\hat{\bbeta} - \hat{\bbeta}_1^{\rm{T}}X_1^{\rm{T}}X_1\hat{\bbeta}_1\right)/p_2$ |
    | Residual | $n-p_1-p_2-1$ | $(\bY - X\hat{\bbeta})^{\rm{T}}(\bY - X\hat{\bbeta})$ | RSS$/(n-p_1-p_2-1)$ |
    | Total | $n-1$ | $\bY^{\rm{T}}\bY - n\bar{Y}^2$ | |

Note that for these *adjusted* sums of squares, in general the extra sum of squares for model (i) and (ii) do not sum to the regression sum of squares. This will only be the case if the columns of $X_1$ and $X_2$ are mutually orthogonal, i.e. $X_1^{\rm{T}}X_2 = \boldsymbol{0}$.
