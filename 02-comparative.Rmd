...
These $t$ equations are not independent; they sum to zero due to the linear dependency between the columns of $X_{2|1}$. Hence, there is no unique solution to $\hat{\boldsymbol{\tau}}$ from equation \@ref(eq:crd-rne). However, we can estimate certain linear combinations of the $\tau_i$, called *contrasts*.
...
a. It explicitly acknowledges that we have fitted a linear model, and so encourages us to check the model assumptions.

a. It is straightfoward to apply adjustments for multiple comparisons.

## Exercises

1. For Example \@ref(exm:one-way), calculate the mean response for each operator and show that the treatment differences and results from hypothesis tests using the results in Section \@ref(contrast-crd) are the same as those found in Section \@ref(r-crd) using `pairwise.t.test`, and `emmeans`.

1. \ [Adapted from @WH2009] The bioactivity of four different drugs $A$, $B$, $C$ and $D$ for treating a particular illness was compared in a study and the following ANOVA table was given for the data:
    ...
    i. The average response from each treatment is as follows: $\bar{Y}_A=66.10$ ($r_A=7$ patients), $\bar{Y}_B=65.75$ ($r_B=8$), $\bar{Y}_C = 62.63$ ($r_C=9$), and $\bar{Y}_D=63.85$ ($r_D=6$). Conduct hypothesis tests for all pairwise comparisons.
...
<!--    Use both the Bonferroni and Tukey methods to perform multiple comparisons of the four treatments at the 0.01 experiment-wise level.
-->
