...
\item
  ...
  The Bonferroni critical value is \(t_{26, 0.01/12} = 3.507\). The Tukey critical value is \(q_{4,26}/\sqrt{2} = 0.304\) (available \texttt{R} as \texttt{qtukey(0.01,\ 4,\ 26)\ /\ sqrt(2)}). Hence under both methods, bioactivity of drugs \(A\) and \(C\), and \(B\) and \(C\), are significantly different.
\item
